...
void App::Load(Platform::String^ entryPoint)
{
	if (m_main == nullptr)
	{
		m_main = std::unique_ptr<XBOX_UWP_VRR_DemoMain>(new XBOX_UWP_VRR_DemoMain(m_deviceResources));
	}
}
...
void App::ExitFullScreenMode()
{
	...
	if (applicationView->IsFullScreenMode)
	{
		...
		// Ensure the window is properly sized after exiting fullscreen
		auto bounds = applicationView->VisibleBounds;
		m_deviceResources->SetLogicalSize(Size(bounds.Width, bounds.Height));

		// Force an immediate update
		m_main->CreateWindowSizeDependentResources();

		OutputDebugString(L"ExitFullScreenMode: Exited fullscreen mode\n");
	}
}
